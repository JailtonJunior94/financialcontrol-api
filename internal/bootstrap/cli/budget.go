...
func RunBudget(dateParam time.Time) {
	...
	container := bootstrapcontainer.Build(db)
	...
	invoices, err := container.InvoiceRepository.GetInvoices(date.AddDate(0, 1, 0))
	if err != nil {
		log.Fatalf("error fetching invoices: %v", err)
	}

	bills, err := container.BillRepository.Get(date)
	if err != nil {
